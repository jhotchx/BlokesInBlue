...
#Load street data
street = sc.textFile('s3://ukpolice/street_LAD_month')
...
df_street_analysis.registerTempTable('street_LAD_month')

df_street_agg_LSOA_year = sqlCtx.sql('select SUBSTRING(Month,1,4) AS Year, LAD_code, LAD_name, SUM(TotalObs) as TotalObs, \
                       ...
                       \
                       from street_LAD_month\
                       \
                       group by SUBSTRING(Month,1,4), LAD_code, LAD_name')

print("Number of records after aggregating to LAD Year level.")
count = df_street_agg_LAD_year.count()
print(count)

#Save a copy of the file at this point into s3
#Change to rdd
rdd_street_agg_LAD_year = df_street_agg_LAD_year.rdd
#Make one file
rdd_street_agg_LAD_year_1 = rdd_street_agg_LAD_year.coalesce(1)
#Save
rdd_street_agg_LAD_year_1.saveAsTextFile('s3://ukpolice/street_LAD_year')
