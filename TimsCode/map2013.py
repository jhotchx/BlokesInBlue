...
unemp2013 = unemployment.loc[(unemployment.Date==2013),['LAD','Unemployment','Date']]
crime2013 = crime.loc[(crime.Year==2013),['LAD','Year','crimes']]
#unemp2013 = unemp2013[pandas.notnull(unemp2013['Unemployment'])]
...
for i in range(len(shapes)):
    try:
        lad_unemployment.append(unemp2013.Unemployment[unemp2013.LAD==lad_names[i]].values[0])
    except IndexError:
        lad_unemployment.append(np.nan)
    try:
        lad_crime.append(crime2013.crimes[crime2013.LAD==lad_names[i]].values[0])
    except IndexError:
        lad_crime.append(np.nan)

#lad_unemployment= unemp2013[unemp2013['LAD'].isin(lad_names)]
...
hover.tooltips = OrderedDict([
    ("Name", "@name"),
    ("Unemployment Rate 2013","@uerate"),
    ("Crime Rate 2013","@crime")
])

output_file("map2013.html", title="LAD GMap with Unemployment 2013", mode="cdn")
show(p)
