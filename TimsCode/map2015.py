...
unemp2015 = unemployment.loc[(unemployment.Date==2015),['LAD','Unemployment','Date']]
crime2015 = crime.loc[(crime.Year==2015),['LAD','Year','crimes']]
#unemp2015 = unemp2015[pandas.notnull(unemp2015['Unemployment'])]
...
for i in range(len(shapes)):
    try:
        lad_unemployment.append(unemp2015.Unemployment[unemp2015.LAD==lad_names[i]].values[0])
    except IndexError:
        lad_unemployment.append(np.nan)
    try:
        lad_crime.append(crime2015.crimes[crime2015.LAD==lad_names[i]].values[0])
    except IndexError:
        lad_crime.append(np.nan)

#lad_unemployment= unemp2015[unemp2015['LAD'].isin(lad_names)]
...
hover.tooltips = OrderedDict([
    ("Name", "@name"),
    ("Unemployment Rate 2015","@uerate"),
    ("Crime Rate 2015","@crime")
])

output_file("map2015.html", title="LAD GMap with Unemployment 2015", mode="cdn")
show(p)
