...
unemp2014 = unemployment.loc[(unemployment.Date==2014),['LAD','Unemployment','Date']]
crime2014 = crime.loc[(crime.Year==2014),['LAD','Year','crimes']]
#unemp2014 = unemp2014[pandas.notnull(unemp2014['Unemployment'])]
...
for i in range(len(shapes)):
    try:
        lad_unemployment.append(unemp2014.Unemployment[unemp2014.LAD==lad_names[i]].values[0])
    except IndexError:
        lad_unemployment.append(np.nan)
    try:
        lad_crime.append(crime2014.crimes[crime2014.LAD==lad_names[i]].values[0])
    except IndexError:
        lad_crime.append(np.nan)

#lad_unemployment= unemp2014[unemp2014['LAD'].isin(lad_names)]
...
hover.tooltips = OrderedDict([
    ("Name", "@name"),
    ("Unemployment Rate 2014","@uerate"),
    ("Crime Rate 2014","@crime")
])

output_file("map2014.html", title="LAD GMap with Unemployment 2014", mode="cdn")
show(p)
